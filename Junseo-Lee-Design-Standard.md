...
Profile entries use a 32 px section heading, an 18 px semibold item title in charcoal, and a 16 px regular description in secondary gray. Keep an 8 px gap between consecutive title, location, and description blocks. This applies across education, experience, leadership, invited talks, honors and awards, and certificates. Dates and location labels share the 14 px regular metadata treatment with 1.50 line height. Place each location or venue label directly below its item title, followed by any description. Location labels use the palette's teal; dates use secondary gray and place names within titles remain charcoal.
...
Keep Honors & Awards separate from Certificates. List repeated honors' terms one per line in the desktop date column. When dates move above entries at widths of 60 rem or less, display the terms inline, separated by commas. Allow natural wrapping if needed. Do not repeat the terms in the description. Use the shared profile list component and its responsive date-list treatment for both sections.
